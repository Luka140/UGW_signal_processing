...
class Measurement:
    ...
    def compare_signals(self, base_index: int, comparison_indices: int | Collection[int], tlim=None):
        ...
        for rx_index in comparison_indices:
            ...
            correlation = spsignal.correlate(base_signal.amplitude_envelope, scaled_signal.amplitude_envelope, mode="full")
            # correlation = spsignal.correlate(base_signal.data, scaled_signal.data, mode="full")
            ...
            time_shift = abs(lags[np.argmax(correlation)]) / base_signal.sample_frequency
            print(f"Time of flight for max correlation: {time_shift} {base_signal.t_unit}")
            ...
            print(f"Wave velocity: {distance / time_shift:.2f}")

            fig, (axtime2, axfrequency2) = plt.subplots(nrows=2, sharex='none', tight_layout=True)
            scaled_shifted_signal = Signal(scaled_signal.time - time_shift, scaled_signal.data, scaled_signal.t_unit, scaled_signal.d_unit)
...
if __name__ == '__main__':
    ...
    measurement = Measurement((0,0), [(18e-3,0), (58e-3, 0.), (98e-3, 0.)], avg_signals[:-1], modes)
